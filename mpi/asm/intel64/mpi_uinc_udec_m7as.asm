...
; uint64_t mpi_uinc_school_bin(uint64_t *r, const uint64_t *a, unsigned int size, uint64_t w)
;
align ARCH_ALIGN_FACTOR
IPPASM mpi_uinc_school_bin,PUBLIC
...
.exit:
   ...
   REST_GPR
   ret
ENDFUNC mpi_uinc_school_bin


;
; borrow, r[:size] = a[:size] - w
; uint64_t mpi_udec_school_bin(uint64_t *r, const uint64_t *a, unsigned int size, uint64_t w)
;

align ARCH_ALIGN_FACTOR
IPPASM mpi_udec_school_bin,PUBLIC
%assign LOCAL_FRAME 0
        USES_GPR rsi,rdi
        USES_XMM
        COMP_ABI 4
